...
export abstract class BaseModule {
  ...
  public async execute(name: string, ...args: any[]) {
    // We need to see if it is a command so we can check aliases
    let hasGuard = false;
    if (name.startsWith("command:")) {
      const command = this.commands.find(
        (x) =>
          x.name === name.split(":")[1] || x.alias.includes(name.split(":")[1])
      )!;
      if (!command) hasGuard = false;
      else hasGuard = this.guardOrder.has(`command:${command.name}`);
    }
    // We need to check if we have a guard order or not.
    if (!hasGuard) {
      if (name.startsWith("command:"))
        await this.handleCommands(name.split(":")[1], args[0] as Message);
      else if (name.startsWith("monitor:"))
        await this.handleMonitor(name.split(":")[1], ...args);
      else if (name.startsWith("task:"))
        await this.handleTask(name.split(":")[1], ...args);
      return;
    }
    ...
    if (name.startsWith("command:")) {
      ...
      order = this.guardOrder.get(`command:${command.name}`)!;
    }

    // we are using this var to know if we can continue the process or not.
    let conti: boolean | undefined = true;
    for (let item of order) {
      if (conti && item.startsWith("guard:"))
        conti = await this.handleGuard(item.split(":")[1], ...args);
      else if (conti && item.startsWith("command:"))
        conti = await this.handleCommands(
          item.split(":")[1],
          args[0] as Message
        );
      else if (conti && item.startsWith("monitor:"))
        conti = await this.handleMonitor(item.split(":")[1], ...args);
      else if (conti && item.startsWith("task:"))
        conti = await this.handleTask(item.split(":")[1], ...args);
      else {
        conti = false;
  ...
  private async handleCommands(name: string, message: Message) {
    try {
      ...
      if (
        !this.commands.find((c) => c.name === name || c.alias.includes(name))
      ) {
      ...
      if (!command) {
        ...
        this._logger.warn(
          `[Command ${name}] Doesn't seem to exist in the module!`
        );
        return false;
      }
  ...
  private async handleGuard(name: string, ...args: any[]) {
    try {
      ...
      if (!guard) {
        this._logger.warn(
          `[Guard ${name}] doesn't seem to exist in the module ${this._options.name}`
        );
        return false;
      }
  ...
  private async handleMonitor(name: string, ...args: any[]) {
    try {
      ...
      if (!monitor) {
        this._logger.warn(
          `[Monitor ${name}] doesn't seem to exist in the module ${this._options.name}!`
        );
        return false;
      }
  ...
  private async handleTask(name: string, ...args: any[]) {
    try {
      ...
      if (!task) {
        this._logger.warn(
          `[Task ${name}] doesn't seem to exist in the module ${this._options.name}!`
        );
        return false;
      }
